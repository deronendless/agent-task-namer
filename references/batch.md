...
- Check the list using the official `list_threads` and project tools first. Results may be capped even when a larger `limit` is requested. Check inventory completeness rather than assuming a recent-task list covers the entire project.
...
A clear user request is enough to establish the topic; a final response is not required.
